...
async fn test_start_and_end() -> shvrpc::Result<()> {
    init_logger();
    let (mut conn_evt_tx, conn_evt_rx) = futures::channel::mpsc::unbounded::<ConnectionEvent>();
    let config = Default::default();
    let app = tokio::spawn(async move {
        mock_run(&config, conn_evt_rx).await
    });

    let (conn_cmd_tx, mut conn_cmd_rx) = unbounded();
    conn_evt_tx.unbounded_send(ConnectionEvent::Connected(conn_cmd_tx))?;
