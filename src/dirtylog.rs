...
mod tests {
    ...
    use crate::{datachange::DataChange, dirtylog::{dirtylog_task, DirtyLogCommand}, util::{init_logger, testing::{run_test, PrettyJoinError, TestStep}}, State};
    ...
    impl TestStep<DirtylogTaskTestState> for TestDirtyLogCommand {
        async fn exec(&self, _client_command_receiver: &mut UnboundedReceiver<ClientCommand<State>>, state: &DirtylogTaskTestState) {
            let cmd = match &self.0 {
                ...
                DirtyLogCommand::Get { .. } => panic!("Cannot clone DirtyLogCommand::Get"),
            };
