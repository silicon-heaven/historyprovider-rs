...
async fn reload_sites(
    ...
    app_state: &Arc<State>,
    periodic_sync_tx: &UnboundedSender<PeriodicSyncCommand>,
...
)
{
    ...
    'main_loop: loop {
        let mut shv_api_version = loop {
            futures::select! {
                event = client_evt_rx.select_next_some() => match event {
                    ClientEvent::Connected(shv_api_version) => break shv_api_version,
                    ClientEvent::ConnectionFailed(connection_failed_kind) => (),
        ...
        'sites_loop: loop {
            let mut state = match reload_sites(shv_api_version.clone(), &client_cmd_tx, &app_state, &periodic_sync_tx).await {
                Some(s) => s,
            ...
            futures::select! {
                event = client_evt_rx.select_next_some() => match event {
                     shvclient::ClientEvent::Connected(new_shv_api_version) => {
                         shv_api_version = new_shv_api_version;
                         continue 'sites_loop;
                    },
                    ClientEvent::Disconnected | ClientEvent::ConnectionFailed(_) => {
                        state.subscribers.clear();
                        state.mntchng_subscribers.clear();
